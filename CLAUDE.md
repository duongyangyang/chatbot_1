...
A single-user PWA personal AI life & work assistant ("Trợ lý AI" / "Life OS"), in Vietnamese. A FastAPI backend serves a self-contained `index.html` SPA and proxies chat to an OpenAI-compatible endpoint, with **native function-calling tools** (time, task CRUD, events, goals, journals, memory search, schedule generation/approval, reviews, profile updates). Conversations, tasks, events, goals, journals, memory, reports, schedules, app config, sync ID, and push subscriptions are persisted in **SQLite** (`memory.db`). For Render/ephemeral filesystems, `memory.db` can be restored/backed up through a GitHub private repo when backup env vars are configured. The app is installable (manifest + service worker), supports scheduled Web Push notifications, and runs **proactive scheduled jobs** (morning planning, daily/weekly/monthly review) that call the LLM server-side, store a report, push a nudge, and surface in-app on next open. A `/db` viewer lets the (non-technical) user read the database in a browser.
...
- **Push notification** (Web Push/VAPID, subscription lưu SQLite), **PWA** installable, **DB viewer** `/db`.
- **Render-safe backup**: nếu set `GITHUB_BACKUP_*`, startup restore `memory.db` từ GitHub private repo khi DB local mất/rỗng; scheduler backup định kỳ.
- **Sync ID**: app cá nhân một người dùng có `sync_id` trong `app_config`, frontend Settings có tạo/copy/tải config server để dùng chung dữ liệu trên thiết bị khác.
...
| `push_subscriptions` | id, endpoint, subscription_json, created_at, updated_at | Web Push subscriptions persisted, reload on startup |
| `app_config` | id=1, base_url, model, api_key_hint, sync_id, updated_at | last-used config + Sync ID; **api_key_hint CHE ở mọi GET** (`***`) |
...
| POST | `/subscribe` | đăng ký Web Push subscription (persist SQLite) |
| GET | `/config` · `/sync-id` · POST `/sync-id` | lấy config public/set Sync ID (không trả API key) |
...
| POST | `/proactive/run` | trigger job thủ công (không push) → hiển thị trong chat |
| GET | `/pending` · POST `/pending/read` | báo cáo chưa đọc + đánh dấu đã xem |
| GET | `/db` · `/debug-db` | xem DB (HTML / JSON, che API key) |
| GET | `/debug-push` `/debug-tools` `/debug-reminders` `/debug-proactive` `/debug-state` `/debug-backup` · `/vapid-public-key` | debug (JSON) |

`/debug-proactive?job=<morning_planning|daily_review|weekly_review|monthly_review>` — chạy job ngay (có push) để test proactive. Dùng `POST /proactive/run {job}` để trigger từ UI (không push, kết quả trả về hiển thị trong chat).
...
- **Proactive jobs (Life OS loop)**: `_register_cron_jobs()` (called from `startup`) arms 4 APScheduler cron jobs in `Asia/Shanghai`: `morning_planning` 07:30, `daily_review` 22:00, `weekly_review` Sun 21:00, `monthly_review` last-day 21:00. Each calls `_run_proactive_job(job_type, push=True)` → `_server_client()` builds an `OpenAI` client from the `app_config` row (last-used key/url/model, env fallback; returns `(None,None)` if no key → job returns `{ok:false, error:"chưa có API key"}` and pushes nothing) → `_build_context_bundle()` gathers open tasks, goals, life_os_state, recent memories → a constrained agent loop (max 4 rounds, `TOOL_SPECS`) runs with a per-job directive → result is stored as a `jobs` row (`status='pending'`) and, when `push`, `send_push()` a short nudge. `_run_morning_planning(client, mdl, push)` parses slots JSON and stores a draft job. **`GET /debug-proactive?job=<type>` runs a job synchronously (with push)**. **`POST /proactive/run {job}` is the manual trigger (no push)** — `_mark_pending_read_by_type` dedups old pending of the same type first, then `_run_proactive_job(job, push=False)`; for review jobs the new job is marked read (so it doesn't reappear in `/pending`), but `morning_planning` with slots stays pending for the Duyệt/Sửa card. The frontend quick-action bar (`triggerJob`) calls this and renders the result as an AI message (morning_planning → schedule card via `renderScheduleCard`).
- **Daily review (interactive + LLM summary)**: `_run_daily_review_interactive()` (cron 22:00) creates a pending `interactive` job + push nudge — the LLM is NOT called yet. The frontend `openDailyReview()` fetches `/daily-review/today` → renders a tick-form (pending/done tasks + "việc khác" textarea) → `submitReview()` posts `/daily-review/submit`. That endpoint completes the ticked tasks, then calls `_llm_summarize_day(done_titles, still_open_titles, other_done, _build_context_bundle())` — a single no-tools LLM call with the daily_review directive — to write the markdown summary (fallback to the hardcoded `parts` block if no key/LLM fails). The summary is saved as a `journal` memory and returned to display in chat. `submitReview` shows a typing indicator during the LLM call.
...
- `GITHUB_BACKUP_TOKEN`, `GITHUB_BACKUP_OWNER`, `GITHUB_BACKUP_REPO`, `GITHUB_BACKUP_BRANCH` (default `main`), `GITHUB_BACKUP_PATH` (default `memory.db`), `BACKUP_INTERVAL_MINUTES` (default `10`) — optional GitHub private repo backup/restore for Render ephemeral filesystem. Token needs Contents read/write only for the backup repo.
...
- **Tasks/reminders/goals/journals/reports/schedules/push subscriptions are in SQLite (`memory.db`)** and survive normal restart; `_reschedule_reminders()` re-arms unfired reminders (including event + schedule reminders, all live in the `reminders` table) on startup, `_register_cron_jobs()` re-arms the 4 proactive cron jobs, and `push_sub_load()` reloads Web Push subscriptions from DB. On Render/free ephemeral filesystem, the local `memory.db` file can disappear across instance replacement/redeploy; configure GitHub backup env vars so startup can restore from private repo before `db_init()` creates a fresh DB. Backup is periodic/best-effort, so data changed after the last successful backup can still be lost; Render Disk or external DB is stronger for zero-loss persistence. The old `assistant.db` is abandoned (fresh start) — do not rely on it.
- **Proactive jobs need a server-side API key.** Scheduled jobs (and `POST /proactive/run`) run without a per-request key, so they use the `app_config` row (last-used config persisted by `/chat`). Until the user has chatted at least once (or `OPENAI_API_KEY` is set in `.env`), `/debug-proactive?job=...` and `/proactive/run` return `error: chưa có API key` and the cron jobs no-op. The frontend `triggerJob` maps this to a friendly "Chưa cài API key" bubble (it deliberately does NOT gate on `isConfigured()`, since the endpoint uses server config not localStorage). Verify with `/debug-state` → `app_config`.
- **Run a single uvicorn worker.** With `--workers >1` or gunicorn multi-worker, push subscriptions survive in SQLite but the in-memory send list, APScheduler reminders, proactive cron jobs, and GitHub backup job would each split across workers → duplicate jobs/nudges/reports/backups or missing in-flight pushes. Use the default single worker.
...
- **LLM trong tổng kết ngày**: `POST /daily-review/submit` giờ gọi `_llm_summarize_day()` (1 lượt LLM không tool, directive `daily_review`) để viết markdown tổng kết từ dữ liệu user tick + "việc khác" + context bundle. Fallback hardcode nếu không key/LLM fail. `submitReview` frontend hiện typing indicator trong lúc đợi.
- **Quick-action bar**: thanh nút phía trên chat (luôn hiện, scroll ngang): Lên lịch mai / Tổng kết ngày / Tổng kết tuần / Tổng kết tháng / Việc hôm nay. `triggerJob(job)` → `POST /proactive/run` (no push) → render kết quả trong chat; morning_planning render card Duyệt/Sửa.
- **`POST /proactive/run`**: trigger job thủ công không push. `_mark_pending_read_by_type` dedup job pending cũ cùng loại (bỏ qua interactive). Review jobs → mark read; morning_planning có slots → giữ pending để duyệt.
- **`push` flag**: `_run_proactive_job(job_type, push=True)` và `_run_morning_planning(client, mdl, push=True)` giờ nhận `push`; cron + `/debug-proactive` vẫn push, `/proactive/run` truyền `push=False`.
- **Message action buttons**: mỗi msg-row có `.msg-actions` (hover-reveal trên desktop, luôn hiện trên touch). AI: 📋 copy + 🔄 regenerate; me: 📋 copy + ✏️ edit (load vào input) + ➤ resend. Raw text lưu trong `_msgText[rowId]`. Regen chỉ kích hoạt trên AI row cuối (`can-regen` class, `_refreshLastAiRegen()`) để không xáo trộn giữa history. Copy có fallback `execCommand` cho PWA non-secure-context.
